...
# 加载训练好的模型
model = joblib.load(open("Machine_best_model.pkl", "rb"))
...
def predict():
    try:
        ...
        # 如果模型期望有20个特征，且我们收到的输入特征少于20个，可以填充剩余的特征为0
        if final_features.shape[1] < 20:
            final_features = np.pad(final_features, ((0, 0), (0, 20 - final_features.shape[1])), 'constant')
